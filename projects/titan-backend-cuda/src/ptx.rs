...
mod tests {
    use super::*;
    use titan_kernel::{BasicBlock, BlockId, Instruction, KernelModule, TargetCompiler, ValueId};
    use titan_types::{DeviceId, KernelId};
    ...
    fn direct_compiler_lowers_add_ir_to_nul_terminated_typed_ptx() {
        let abi = super::super::elementwise_add_f32_abi();
        let bytes = super::super::CudaCompiler.compile(&add_ir(abi.clone()), &abi, &fingerprint("sm_86")).unwrap();
    ...
    fn lowering_rejects_invalid_abi_non_global_pointer_and_old_target() {
        ...
        assert!(matches!(lower(&add_ir(abi.clone()), &invalid_abi, &fingerprint("sm_86")), Err(KernelError::InvalidAbi(_))));
        ...
        assert!(matches!(lower(&non_global, &abi, &fingerprint("sm_86")), Err(KernelError::Unsupported(_))));
        assert!(matches!(lower(&add_ir(abi.clone()), &abi, &fingerprint("sm_61")), Err(KernelError::Unsupported(_))));
    }
}
